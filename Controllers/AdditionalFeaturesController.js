...
const FeaturesChalet = require("../Models/AdditionalFeatures");
...
exports.createFeature = async (req, res) => {
  try {
    ...
    const newFeaturePromise = AdditionalFeatures.create({ FeatureName,lang});

    const cacheDeletePromises = [client.del(`addFeature:page:1:limit:20`)];
    ...
    await client.set(`addFeature:${newFeature.id}`, JSON.stringify(newFeature), {
      EX: 3600,
    });
  ...
  try {
    ...
    if (lang && !validLanguages.includes(lang)) {
      return res.status(400).json(
        ErrorResponse("Invalid language", ["The provided language is not valid."])
      );
    }

    
    const cacheKey = `addFeature:page:${page}:limit:${limit}:lang:${lang || 'all'}`;
    ...
    const featureEntries = await AdditionalFeatures.findAll({
    ...
    return res.status(200).json(featureEntries);
  } catch (error) {
...
exports.getFeatureById = async (req, res) => {
  try {
    ...
    const cacheKey = `addFeature:${id}:lang:${lang || 'all'}`;
    ...
    const whereClause = lang ? { id, lang } : { id };

    const feature = await AdditionalFeatures.findOne({
      attributes: ["id", "FeatureName", "lang"],
      where: whereClause,
    });
...
exports.updateFeature = async (req, res) => {
  try {
    ...
    const feature = await AdditionalFeatures.findByPk(id);
    if (!feature) {
      return res
        .status(404)
        .json(
...
exports.deleteFeature = async (req, res) => {
  try {
    ...
    await client.del(`addFeature:${id}`);
    await client.del(`addFeature:all`); 
