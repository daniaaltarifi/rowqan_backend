...
exports.getAllChaletProps = async (req, res) => {
  try {
    ...
    client.del(`chaletProps:lang:${lang}:page:${page}:limit:${limit}`);
    ...
    const properties = await Chalets_Props.findAll({
      where: whereClause,
      include: [{ model: Chalets, attributes: ["title"] }],
      order: [["id", "DESC"]],
    ...
    res
      .status(500)
      .json(
        ErrorResponse("Failed to fetch Chalet properties", [
  ...
  try {
    if (lang && !["en", "ar"].includes(lang)) {
      return res
        ...
        .json(
          ErrorResponse(
            'Invalid language parameter. Language must be "en" or "ar".'
          )
        );
    }
    ...
    if (lang && property.lang !== lang) {
      return res
        .status(400)
        .json(ErrorResponse("Language does not match the record."));
    }
  ...
  try {
    ...
    if (lang && !["en", "ar"].includes(lang)) {
      return res
        .status(400)
        .json(
          ErrorResponse("Invalid language. Allowed values are 'en' or 'ar'")
        );
    }
    ...
    if (Chalet_Id && Chalet_Id !== property.Chalet_Id)
